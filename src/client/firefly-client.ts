...
export class FireflyApiClient {
  ...
  private async executeWithRetry<T>(
    ...
    for (
      let attempt = 0;
      attempt <= this.config.firefly.retryAttempts;
      attempt++
    ) {
      try {
        ...
        if (
          error instanceof FireflyApiError && error.status >= 400 &&
          error.status < 500
        ) {
          throw error;
        }
        ...
        if (attempt < this.config.firefly.retryAttempts) {
          ...
          logger.warn(
            `Request failed (attempt ${attempt + 1}/${
              this.config.firefly.retryAttempts + 1
  ...
  async getAccounts(type?: string): Promise<ApiResponse<Account[]>> {
    ...
    return await this.request<ApiResponse<Account[]>>("/api/v1/accounts", {
      params,
    });
  }
  ...
  async getTransactions(
    ...
    return await this.request<ApiResponse<Transaction[]>>(
      "/api/v1/transactions",
      { params },
    );
  }
