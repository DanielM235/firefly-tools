...
async function copyConfigTemplate(): Promise<void> {
  ...
  try {
    ...
    console.log('📝 Please edit config/config.json with your Firefly III details');
  } catch (error) {
...
function promptUser(question: string): Promise<string> {
  return new Promise((resolve) => {
    ...
    // In a real implementation, you'd use Deno's stdin
...
async function interactiveSetup(): Promise<void> {
  ...
  console.log('Please provide your Firefly III details:\n');

  const baseUrl = await promptUser('Firefly III URL (e.g., https://firefly.example.com): ');
  const apiToken = await promptUser('Personal Access Token: ');
  ...
  if (baseUrl && apiToken) {
    ...
    try {
      await Deno.writeTextFile('./config/config.json', JSON.stringify(config, null, 2));
...
async function testConfiguration(): Promise<void> {
  ...
  try {
    ...
    const { FireflyApiClient } = await import('../src/client/firefly-client.ts');
    ...
    console.error('❌ Configuration test failed:', error instanceof Error ? error.message : error);
  }
}
...
async function main(): Promise<void> {
  ...
  if (hasConfig) {
    ...
    console.log('💡 Please copy config/config.example.json to config/config.json');
    console.log('📝 Then edit it with your Firefly III details');
  }

  console.log('\n📚 Next steps:');
  console.log('1. Edit config/config.json with your API details');
  console.log('2. Run: deno run --allow-read --allow-net --allow-env examples/basic-usage.ts');
